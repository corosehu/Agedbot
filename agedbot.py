...
main_kb.add(KeyboardButton("Browse Products"))
main_kb.add(KeyboardButton("My Orders"))
main_kb.add(KeyboardButton("Support"))
...
async def start_cmd(msg: types.Message):
    ...
    if uid not in users:
        users.append(uid)
        save_all()
    await msg.reply("Welcome! Choose an option.", reply_markup=main_kb)

# ------------------------ BROWSE PRODUCTS ------------------------

@dp.message_handler(lambda m: m.text == "Browse Products")
async def browse(msg: types.Message):
    if not products:
        return await msg.reply("No products available.")

    text = "Available Products:\n\n"
    for p in products:
        text += f"ID: {p['id']} | {p['name']} | Price: ₹{p['price']}\n"
    text += "\nTo order: /order <product_id> <qty>"
...
async def order_cmd(msg: types.Message):
    try:
        ...
        return await msg.reply("Format: /order <product_id> <qty>")
    ...
    pay_text = f"""Order Created!

Order ID: {order_id}
Amount: ₹{amount}

Send: PAYMENT <order_id> after you pay and attach screenshot."""
...
@dp.message_handler(lambda m: m.text.startswith("PAYMENT"))
async def handle_payment(msg: types.Message):
    try:
        _, oid = msg.text.split()
        oid = int(oid)
    except:
        return await msg.reply("Format: PAYMENT <order_id>")
...
async def my_orders(msg: types.Message):
    my = [o for o in orders if o["user"] == msg.from_user.id]
    if not my:
        return await msg.reply("No orders yet.")

    text = "Your Orders:\n\n"
    for o in my:
        text += f"ID {o['id']} | Qty {o['qty']} | ₹{o['amount']} | {o['status']}\n"
...
async def support_msg(msg: types.Message):
    await msg.reply("Send your issue. Admin will contact you.")
